...
describe Puppet::Type.type(:netapp_nfs_export).provider(:sevenmode) do
  ...
  describe "when asking exists?" do
    it "should return true if resource is present" do
      ...
      nfs_export_volume.provider.should be_exists
    end

    it "should return false if resource is absent" do
      nfs_export_volume.provider.set(:ensure => :absent)
  ...
  describe "when destroying a resource" do
    it "should be able to destroy a volume export" do
      ...
      nfs_export_volume.provider.set(:name => '/vol/volume', :path => '/vol/volume')
      nfs_export_volume.provider.expects(:edel).with('persistent', 'true', 'pathnames', is_a(NaElement))
    ...
    it "should be able to destroy a qtree export" do
      ...
      nfs_export_qtree.provider.expects(:edel).with('persistent', 'true', 'pathnames', is_a(NaElement))
      nfs_export_qtree.provider.destroy
  ...
  describe "when modifying a resource" do
    it "should be able to modify an existing export path" do
      ...
      nfs_export_volume_path.provider.set(:name => '/vol/volume', :ensure => :present, :path => :absent)
    ...
    it "should be able to modify an existing export readonly list" do
      ...
      nfs_export_volume_path[:readonly] = ['server1','192.168.150.50','192.168.150.51']
      nfs_export_volume_path.provider.expects(:emodify).with('persistent', 'true', 'rule', is_a(NaElement))
      nfs_export_volume_path.provider.flush
    end
    ...
    it "should be able to modify an existing export readwrite list" do
      ...
      nfs_export_volume_path[:readwrite] = ['server1','192.168.150.50','192.168.150.51']
      nfs_export_volume_path[:readonly] = ['all_hosts']
      nfs_export_volume_path.provider.expects(:emodify).with('persistent', 'true', 'rule', is_a(NaElement))
      nfs_export_volume_path.provider.flush
    end
  end
